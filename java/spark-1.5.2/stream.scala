...
class StreamHandler(serializer: Serializer, tracker: JVMObjectTracker, preCommandHooks: List[Runnable] = List.empty) {
  ...
  def handleMethodCall(
    ...
    returnJObjRef: Boolean): Unit = {
      logger.log(s"handleMethodCall: ${preCommandHooks.size}")
